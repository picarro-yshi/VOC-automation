...
ParseCRDSName_NoInstrNum = r'([A-Z]+)-([0-9]{4})([0-9]{2})([0-9]{2})-([0-9]{2})([0-9]{2})([0-9]{2})Z?-([^.]+).(dat|h5)$'
ParseCRDSName_AVX80 = r'^([A-Z]+)([0-9]{2})([0-9]{4})-([0-9]{4})([0-9]{2})([0-9]{2})-([0-9]{2})([0-9]{2})([0-9]{2})Z?-([^.]+).(dat|h5)$'
class CRDSFileName():
...
def parseFileName(fn):
    ...
    if match is None:
        match = re.match(ParseCRDSName_NoInstrNum, fn)
        if match is None:
            match = re.match(ParseCRDSName_AVX80, fn)
            if match is None:
                return None
            else:
                U = list(match.groups())
...
class PicarroZip(object):
    ...
    def extractZipFile(self, targetDIR, agelim=1e6, maxcount=1e9, verbose=True):
        ...
        if (self.ftype is None) and verbose:
            ...
            if verbose: print (f'Extracting archive: {self.fn}')
            ...
            if AGE <= agelim:
                ...
                if not(os.path.exists(targetfn)):
                    ...
                    try:
                        ...
                        if verbose:
                            print('unable to change file modification datetime')
...
class RetrievePrivateData(object):
    ...
    def indexFiles(self):
        ...
        print('%d h5 files indexed' % (len(self.files)))
    ...
    def loadPrivateData(self, startTime, duration, colList=None):
        s, e, good = self.retrieveFileNames(startTime, duration)
        if not(good):
            print('Private data archive does not cover date ranges')
            return None
        print(s, e, len(self.files)-1)
        if colList is not None:
            if 'timestamp' not in colList:
                colList.append('timestamp')

        outputs = defaultdict(list)
        for fn in self.files[s:e+1]:
            ht, dat = LF.loadFileH5(fn)
            def getColumn(tag):
                if tag in ht:
                    return dat[:, ht.index(tag)]
                else:
                    return []
            if colList is not None:
                theseTags = colList
            ...
            for tag in theseTags:
                outputs[tag].extend(getColumn(tag))
