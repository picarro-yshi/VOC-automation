...
## Yilin Shi | Picarro | last update: 2022.6.14
...
spec_xt = 180  ## min, set Tab3 x axis time window duration to 3h (actual is 3h10' due to latency); >3h will crash
...
class Window(QWidget):
    ...
    def layout5(self):
        ...
        self.e87 = QLineEdit('4')  # zero 2 < zero1 + pct*sigma
        ...
        try:
            if self.func32():
                try:
                    ...
                    self.plot1()
                    self.lb10.setText('Plotting spectrum on Tab 3. (Time window = 3h)')
                except:
    ...
    def func_create(self):   #create new exp.
        ...
        if fname == '':
            ...
            self.lb50.setText('Error, %s folder not found on R/Data drive.\nPlease check location.\n'%gas)
        ...
        if tag:  #data key must exist, func32
            ...
            if key != keyplot:
                tag = 0
                self.lb50.setText('Error, please select "%s" \n'
        ...
        if tag:
            try:
                ...
                self.lb302.setText(' ')  # tab3 note
                self.lb29.setText('0.0')  # weight
        ...
        for j in range(20):
            ...
            if dm['source'] == analyzer_source:
                ...
                if 'time' not in dm['data']:
                    self.lb50.setText("Missing datakey 'time'\n\n")
                elif a not in dm['data']:
                    self.lb50.setText("Missing datakey '%s'\n\n"%a)
                    ...
                    if self.rb51.isChecked(): # check datakey: MFC1_flow, MFC2_flow
                        ...
                        if 'MFC1_flow' not in dm['data']:
                            self.lb50.setText("Missing datakey dm['MFC1_flow']\n\n")
                        elif 'MFC2_flow' not in dm['data']:
                            self.lb50.setText("Missing datakey dm['MFC2_flow']\n\n")
                        else:
                            tag = 1
                    else:
                        tag = 1
    ...
    def track_baseline1(self):  # monitor baseline for droplet, data sent by animation
        try:
            ...
            if self.e78.currentText() =='00' and self.e79.currentText() == '00':
                self.lb50.setText(self.note1+', now: %.6E'%zero2)
    ...
    def func64(self):  ## stop sending data to analyzer, record time
        try:
            ...
            try:
                ...
                print('R drive connection lost. Internet may temporarily down.')
    ...
    def func72(self):  ## switch tank, record time
        try:
            if int(time.time()) < self.epoch2:  # current time < epoch2
                ...
                self.lb50.setText(self.epoch3note)
    ...
    def track_baseline2(self):  # monitor baseline for gas tank, data sent by animation
        try:
            ...
            if sigma2 > self.sigma1*1.1:   #1.05
                if self.e78.currentText() == '00' and self.e79.currentText() == '00':
                    self.lb50.setText(self.epoch3note+", now: %.6E"%(sigma2))
