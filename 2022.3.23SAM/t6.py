...
class Window(QWidget):
    ...
    def mainlayout(self):
        ...
        self.tabs.addTab(self.tab2, "   ⬥ Known VOC Real Time   ")
        self.tabs.addTab(self.tab3, "  ⬥ Unknown VOC Real Time  ")
    ...
    def plot1(self):     # start run csv data, plots shown on 2 figures
        try:
            ...
            ax2 = self.figure1.add_subplot(221)
            ax3 = self.figure1.add_subplot(222)
            ax4 = self.figure1.add_subplot(223)
            ax1 = self.figure2.add_subplot(321)
            ax5 = self.figure2.add_subplot(322)
            ...
            def animate(i):
                ...
                # ax2.plot(x, y0, linewidth=0.5,label='MFC10(200SCCM)')
            ...
            def animate2(i):
                ...
                if len(x2) == self.view_point:  ## x-axis number
                    x2.pop(0)
                    y1.pop(0)
                    # y2.pop(0)
                    # y3.pop(0)
                ...
                ax1.plot(x2, y1,  linewidth=0.5, c='k')  # measured
                ax1.plot(x2, yy1, linewidth=0.5, c='g')  #target
                # ax2.plot(x, y2,  linewidth=0.5, c='k')
                # ax2.plot(x, yy2, linewidth=0.5, c='g')
                # ax3.plot(x, y3,  linewidth=0.5, c='k')
                # ax3.plot(x, yy3, linewidth=0.5, c='g')
                # ax4.plot(x, y4,  linewidth=0.5, c='k')
                # ax4.plot(x, yy4, linewidth=0.5, c='g')
                ax5.plot(x2, y5,  linewidth=0.5, c='k')
                ax5.plot(x2, yy5, linewidth=0.5, c='g')
                ...
                ax1.set_xticks(xx2, xmak2)
                # ax2.set_xticks(xx, xmak)
                # ax3.set_xticks(xx, xmak)
                # ax4.set_xticks(xx, xmak)
                ax5.set_xticks(xx2, xmak2)
            ...
            if cycle:
                ...
                self.anim2 = FuncAnimation(self.figure2, animate2, frames=self.gen_mfc(total, cycle), interval=1000, repeat=False)
            else:
                print('infinity')
                self.anim1 = FuncAnimation(self.figure1, animate, frames=self.gen_mfc(total), interval=1000) #, repeat=False)
                self.anim2 = FuncAnimation(self.figure2, animate2, frames=self.gen_mfc(total), interval=1000) #, repeat=False)
            self.canvas1.draw()
            self.canvas2.draw()
