...
class Window(QWidget):
            ...
            def animate(i):
                ...
                if dm['source'] == analyzer_source:
                    ...
                    if ((a.split(":")[1] == '59' and b.split(":")[1] == '00') or
                            (a.split(":")[1] == '29' and b.split(":")[1] == '30') or
                            (a.split(":")[1] == '14' and b.split(":")[1] == '15') or
                            (a.split(":")[1] == '44' and b.split(":")[1] == '45')):
                        xx.append(t)
    ...
    def func52(self):  ## change MFC1
        try:
            port_ali = self.e202.text()
            F1 = float(self.e53.text())  # bubbler line
            flow_controller1 = FlowController(port=port_ali, address='A')
    ...
    def func73(self):  ## switch tank, record time
        try:
            ...
            if tank_conc == '':
            ...
            elif epoch < self.epoch2:
                ep = time.strftime('%Y%m%d %H:%M:%S', time.localtime(self.epoch2))
                self.lb50.setText('Please wait at least 30 min,\nuntil %s:%s to add sample.\n' % (ep[9:11], ep[12:14]))
            else:
                self.zero1 = np.mean(self.baseline[:-10])
                self.sigma1 = np.std(self.baseline[:-10])
                print('zero1')
                ...
                self.b61.setEnabled(False)
                self.b64.setEnabled(True)
                self.lb50.setText("Sample added at %s:%s!\nPlease run until baseline is stable.\n"
                                  "You will be notified by then." % (t2, t3))
        except:
            self.lb50.setText('Error record add sample time.\n\n')
    ...
    def track_baseline2(self):  # monitor baseline for gas tank, data sent by animation
        try:
            ...
            if sigma2 > self.sigma1*1.02:
                print('not leveled yet')
            elif self.e74.text() == '':
                t1 = time.strftime("%Y%m%d")
                t2 = time.strftime("%H")
                t3 = time.strftime("%M")
                epoch = datetime.datetime(int(t1[:4]), int(t1[4:6]), int(t1[-2:]), int(t2), int(t3)).timestamp()-1800
                ep = time.strftime('%Y%m%d %H:%M:%S', time.localtime(epoch)) #20220329 11:02:15
                tb1 = ep[:8]  # 30 min before
                tb2 = ep[9:11]
                tb3 = ep[12:14]
