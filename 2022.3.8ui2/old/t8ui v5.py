...
spec_xt = 300             ## min, spectrum plot animation: x axis time window duration 400'=6h40', 500'=8h20'
...
class Window(QWidget):
    ...
    def layout1(self):
        ...
        self.b12.setToolTip("Plot selected data in extra window; may not work on Windows.\n"
                            ...
                            "Maximum plot time is 5hx2, to prevent program crash.\n"
    ...
    def layout4(self):
        ...
        self.b61.setToolTip("Start experiment, send Alicat data to analyzer.\n"
                            ...
                            "You can adjust start time and will be saved\n"
    ...
    def specplot(self):  ## may stop after a while on Wifi, but fine with Ethernet cable
        if self.func32():
            try:
                self.lb10.setText('Plotting spectrum on Tab 3. (Max. duration 5h x2)')
        ...
        try:
            ...
            def animate(i):
                ...
                if dm['source'] == analyzer_source:
                    ...
                    baseline_pt = int(baseline_time / sec_pt)
    ...
    def func61(self):  ## start experiment, send Alicat data to analyzer, record time
        try:
            ...
            self.epoch2 = epoch1 + baseline_time*60 + 600   ##30 min later: 20 min baseline+10 min, see spike when turn on bubble line
    ...
    def track_baseline(self):  # monitor baseline, data sent by animation
        try:
            ...
            if zero2 < self.zero1 + self.sigma1 * pct: #record value when sees baseline+n*sigma
                ...
                self.lb50.setText('Concentration has dropped below baseline+%s sigma.\n'
                                  'You may stop experiment after a while.\n'
    ...
    def func64(self):    ## stop sending data to analyzer, record time
        try:
            ...
            if t1 == '':   #no data, write current time
                t1 = time.strftime("%Y%m%d")
                t2 = time.strftime("%H")
                t3 = time.strftime("%M")
                self.e77.setText(t1)  ## '20211124'
        ...
        if fname == '':
            ...
            try:
                ...
                if os.path.exists(fnr):
                    ...
                    self.b61.setEnabled(True)
                    lb50text = 'Experiment %s created!\nYou may start the experiment now.\n'%(ta1+suffix)
                    if len(self.baseline) == 0:
                        lb50text += 'You may also want to start Spectrum View Real Time.'
                    self.lb50.setText(lb50text)
            except:
                self.lb50.setText('Error creating experiment.\n\n')
    ...
    def func72(self):  ## start experiment, half hour baseline, record time
        try:
            ...
            self.b63.setEnabled(True)
            self.b66.setEnabled(True)
            self.lb50.setText("Experiment started at %s:%s!\nPlease wait at least 30 min, until %s:%s to add sample.\n"
                               % (t2, t3, ep[9:11], ep[12:14]))
        except:
            self.lb50.setText('Error start experiment.\n\n')

    ## parameters should be valid before sent to function
    def func73(self):    ## switch tank, half hour IPA, record time
        try:
            t1 = time.strftime("%Y%m%d")
            t2 = time.strftime("%H")
            t3 = time.strftime("%M")
            ...
            if epoch < self.epoch2:
                ...
                self.epoch3 = epoch + 2400  ## 40 min later, 10 min change time + >30 min baseline
                ep = time.strftime('%Y%m%d %H:%M:%S', time.localtime(self.epoch3))
                self.b61.setEnabled(False)
                self.b64.setEnabled(True)
                self.lb50.setText("Sample added at %s:%s!\nPlease run for at least 40 mins until %s:%s.\n"
                                  % (t2, t3, ep[9:11], ep[12:14]))
        except:
            self.lb50.setText('Error record add sample time.\n\n')
    ...
    def func74(self):    ## stop, record time, must be clicked
        try:
            ...
            epoch = datetime.datetime(int(t1[:4]), int(t1[4:6]), int(t1[-2:]), int(t2), int(t3)).timestamp()
            if epoch < self.epoch3:
                ep = time.strftime('%Y%m%d %H:%M:%S', time.localtime(self.epoch3))
                self.lb50.setText('Please wait at least 40 mins\nuntil %s:%s to end the experiment.\n' %(ep[9:11], ep[12:14]))
            else:
                fname = self.e81.toPlainText()
                gas = self.e82.text()
                ta1 = self.e71.text()
        ...
        try:
            ...
            if (not ta1.isdigit()) or (len(ta1) != 8):
                ...
                if self.rb51.isChecked():
                    ...
                    jupyternotebook_dp(fnr, gas, cid, weight, MW, pct, ta1, ta2, ta3, tb1, tb2, tb3, tc1, tc2, tc3,row1, savefig=True)
