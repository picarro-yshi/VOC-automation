...
spec_x = 1500             ## spectrum plot x axis data number
baselinetime = 1200       ## 20 min
analyzer_source = 'analyze_VOC_broadband_custom'   ## listener data key
total_data = 1900         ## total points plotted in spectrum viewer real time
...
class Window(QWidget):
    ...
    def layout4(self):
        ...
        self.b63.setToolTip("Stop flow to add sample, then restore the flow.\n"
                            "After adding the sample, click this button to\n"
                            "record time (format yyyymmdd hh:mm) and weight.")
    ...
    def layout5(self):
        ...
        self.e87 = QLineEdit('1')     # zero 2 < zero1 + pct*sigma
    ...
    def plot1(self):
        try:
            ...
            print('Error. Queue might be empty')
    ...
    def func61(self):  ## start experiment, send Alicat data to analyzer, record time
        try:
            ...
            self.epoch2 = epoch1 + baselinetime      ##20 min later  20220301 11:00:00
           ...
           #### get baseline before start
            ...
            self.lb50.setText("Experiment started at %s:%s!\nPlease wait at least until %s:%s to add sample.\n"
                              "Baseline is about %.4f"% (t2, t3, ep[9:11], ep[12:14], self.zero1))
        except:
    ...
    ## parameters should be valid before sent to function
    def func63(self):    ## add sample, record time, weight
        try:
    ...
    def func65(self):
        # check R, alicat connection, fill in time, create folder,enable buttons,
        fname = self.e81.toPlainText()
        gas = self.e82.text()
        cid = self.e83.text()
        weight = self.e85.text()
