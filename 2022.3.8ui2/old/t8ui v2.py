...
spec_x = 2200             ## spectrum plot x axis data number, 1500=4.2h
analyzer_source = 'analyze_VOC_broadband_custom'   ## listener data key
total_data = 1900         ## total points plotted in spectrum viewer real time, to prevent Python collaps
baseline_time = 1200     ## s, 20 min sample baseline time
baseline_pt = int(baseline_time/11)        ## after sample baseline points (listener speed ~10s/point)
baseline = []            # 20 min baseline range value storage
...
class Window(QWidget):
    ...
    def layout4(self):
        ...
        self.e54 = QComboBox()
        self.e54.addItems(["50", "20", "80", "100"])
        ...
        self.b63.setToolTip("Steps:Stop flow ->add sample ->restore flow\n"
                            ...
                            "Sample add time, format yyyymmdd hh:mm")
        self.b63.clicked.connect(self.func63)
        ...
        self.b64.setToolTip("Stop experiment.\nStop sending Alicat data to analyzer.\n"
                            "Record end time, format yyyymmdd hh:mm.")
        self.b64.clicked.connect(self.func64)
        ...
        # self.lb65 = QLabel('(Experiment time format: yyyymmdd hh mm)')
        # self.lb65.setStyleSheet('color: grey')
        self.b65=QPushButton("Create New Exp.", self)
        self.b65.clicked.connect(self.func65)
        self.b65.setStyleSheet("font: bold")
        self.b65.setToolTip("Create a folder named with above start date\n"
                            "(leave blank if you wish to use today's date)\n"
        ...
        self.g7 = QGridLayout()
        # self.g7.addWidget(self.lb65,  0, 0, 1, 2)
        self.g7.addWidget(self.b65,   1, 0)
        self.g7.addWidget(self.lb50,  2, 0, 1, 3)
        ...
        ## monitor baseline
        self.timer2 = QTimer()
        self.timer2.setInterval(300000)  ## ms, 5 mins
        self.timer2.timeout.connect(self.func66)
    ...
    def layout5(self):
        ...
        self.g8.addWidget(self.lb89,  8, 0)  # %
        ...
        self.g8.addWidget(self.e87,   8, 1, 1, 2)  # %
    ...
    def func12(self):   ## not working on Windows, plot not shown
        if self.func32():
            try:
                ...
                t8.specviewer(host, port_out, gas, spec_x)
            except:
    ...
    def specplot(self):  ## may stop after a while on Wifi, but fine with Ethernet cable
        if self.func32():
            try:
                ...
                self.figure1.clear()
                baseline = []  # 20 min baseline range
                self.plot1()
            except:
                self.lb10.setText('Error. Analyzer data out port not ready.')
        else:
    ...
    def plot1(self):
        try:
            ...
            xt = []  ## real time 19:30\nJan-3-2022   Mon Jan  3 17:30:01 2022
            y = []
            xx = []  ## x labels that will be marked

            host = self.e201.text()  ## IP address
            gas = self.e11.currentText()
            # gas = 'MFC2_flow'
            ...
            def animate(i):
                ...
                if dm['source'] == analyzer_source:
                    ...
                    if len(baseline) == baseline_pt:   ## baseline after sample range
                        baseline.pop(0)
                    baseline.append(dm['data'][gas])
            ...
            # self.anim1 = FuncAnimation(self.figure1, animate, frames=gen(), interval=1000)
    ...
    def func52(self):   ## change MFC1
        self.lb50.setText("change MFC1 flow rate.\n\n")
    ...
    def func61(self):  ## start experiment, send Alicat data to analyzer, record time
        try:
            ...
            epoch1 = datetime.datetime(int(ta1[:4]), int(ta1[4:6]), int(ta1[-2:]), int(t2), int(t3)).timestamp()
            self.epoch2 = epoch1 + baseline_time      ##20 min later  20220301 11:00:00
           ...
           # #### get baseline before start
           #  y=[]
           #  host = self.e201.text()  ## IP address
           #  gas1 = self.e11.currentText()
           #  dm_queue = Queue(180)  ## data manager
           #  listener = Listener(dm_queue, host, port_out, StringPickler.ArbitraryObject, retry=True)
           #  for i in range(20):    ### small sampling
           #      dm = dm_queue.get(timeout=5)
           #      if dm['source'] == analyzer_source:
           #          y.append(dm['data'][gas1])
           #  # print(len(y))
           #  self.zero1 = np.mean(y)
           #  self.sigma1 = np.std(y)
           #  print('zero1')
           #  print(self.zero1)
           #  print(self.sigma1)
            ...
            #                   "Baseline is about %.8f"% (t2, t3, ep[9:11], ep[12:14], self.zero1))
            self.lb50.setText("Experiment started at %s:%s!\nPlease wait at least until %s:%s to add sample.\n"
    ...
    ## parameters should be valid before sent to function
    def func63(self):    ## add sample, record time, weight, get baseline 1
        try:
            ...
            epoch = datetime.datetime(int(t1[:4]), int(t1[4:6]), int(t1[-2:]), int(t2), int(t3)).timestamp()
            if epoch < self.epoch2:
                ep = time.strftime('%Y%m%d %H:%M:%S', time.localtime(self.epoch2))
                self.lb50.setText('Please wait at least 20 min,\nuntil %s:%s to add sample.\n' %(ep[9:11], ep[12:14]))
            elif weight == '':
                self.lb50.setText('Please type in sample weight.\n\n')
            else:
                ## get baseline 1:
                self.zero1 = np.mean(baseline)
                self.sigma1 = np.std(baseline)
                print('zero1')
                print(self.zero1)
                print(self.sigma1)
                ...
                self.lb50.setText("Sample added at %s:%s.\n"
                                  "Baseline before sample is about %.8f\n" %(t2, t3, self.zero1))
        except:
            self.lb50.setText('Error record add sample time.\n\n')

    def func66(self):  #monitor baseline
        try:
            ...
            if zero2 < self.zero1 + self.sigma1 * pct:
                if self.e77.text() == '':   #record value when first time see baseline+n*sigma
                    fname = self.e81.toPlainText()
                    gas = self.e82.text()
                    ta1 = self.e71.text()
                    suffix = self.e70.text()
                ...
                self.lb50.setText('Concentration has dropped below baseline+%s sigma.\n'
                                  'You may stop experiment anytime from now.\n'
                                  'Baseline before: %.8f, now: %.8f"' % (int(pct), self.zero1, zero2))
        except:
    ...
    #                 f.write(t2 + '\n')
    #                 f.write(t3 + '\n')
    #                 self.lb50.setText('Concentration has dropped below baseline+%ssigma.\n'
    #                                   'You may stop experiment anytime from now on.\n'
    #                                   'Baseline is about %.4f"' % (pct, zero2))
    #     except:
    ...
    def func64(self):    ## stop sending data to analyzer, record time
        try:
            ...
            self.timer.stop()
            self.lb50.setText('Experiment stopped at %s:%s.\n\n' % (t2, t3))
        except:
    ...
    def func80(self):
        try:
            ...
            suffix = self.e70.text()     ## ''
            fnr = os.path.join(fname, gas, ta1 + suffix)
            # /mnt/r/crd_G9000/AVXxx/3610-NUV1022/R&D/Calibration/176 - Acetic Acid/20211124c
            ...
            if (not ta1.isdigit()) or (len(ta1) != 8):
                ...
                jupyternotebook(fnr, gas, cid, weight, MW, pct, ta1, ta2, ta3, tb1, tb2, tb3, tc1, tc2, tc3, row1)
