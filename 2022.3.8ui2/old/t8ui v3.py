...
spec_x = 2000             ## spectrum plot animation: x axis data number, 1500=4.2h
total_data = 10000        ## spectrum plot animation: total points to prevent Python colapse
baseline_time = 1200      ## baseline: s, 20 min sample baseline time
baseline_pt = int(baseline_time/10)  ## baseline: after sample baseline points (listener speed ~10s/point, or 5s??)
...
class Window(QWidget):
    ...
    def layout1(self):
        ...
        combolist = ['broadband_gasConcs_176', 'broadband_gasConcs_280', 'broadband_gasConcs_962', 'H2O']
        try:
    ...
    def layout4(self):
        ...
        self.b61.setToolTip("Start experiment, send Alicat data to analyzer.\nRecord start time, format yyyymmdd hh:mm.\n"
                            "You can adjust start time and will be saved finally\n"
                            "when you click Add Sample button.")
        ...
        self.b64.setToolTip("End this experiment.\nStop sending Alicat data to analyzer.\n"
                            "Record current time as end time, format yyyymmdd hh:mm.")
        ...
        self.timer2.timeout.connect(self.track_baseline)
    ...
    def func12(self):   ## not working on Windows, plot not shown
        if self.func32():
            try:
                ...
                t8.specviewer(host, port_out, gas, analyzer_source, spec_x)
            except:
    ...
    def specplot(self):  ## may stop after a while on Wifi, but fine with Ethernet cable
        if self.func32():
            try:
                self.lb10.setText('Plotting spectrum on Tab 3. (Max 8h, after then start again)')
    ...
    def func52(self):   ## change MFC1
        try:
            port_ali = self.e202.text()
            F1 = int(self.e53.text())  # bubbler line
            flow_controller1 = FlowController(port=port_ali, address='A')
    ...
    def func61(self):  ## start experiment, send Alicat data to analyzer, record time
        try:
            ...
            self.epoch2 = epoch1 + baseline_time + 600   ##30 min later: 20 min baseline+10 min, see spike when turn on bubble line
    ...
    def func63(self):    ## add sample, record time, weight, get baseline 1
        try:
            ...
            if epoch < self.epoch2:
                ...
                ## get baseline 1:
                self.zero1 = np.mean(self.baseline)
                self.sigma1 = np.std(self.baseline)
                print('zero1')
                print(self.zero1)
                print(self.sigma1)
    ...
    def func66(self):  #stop current experiment
        reply = QMessageBox.question(self, 'Warning',
                    "Are you sure to abandon current experiment?\n No data will be saved.", QMessageBox.StandardButton.Yes |
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                if self.e74.text() != '' and self.e77.text() == "":
                    self.lb50.setText('Experiment stopped. You may stop the flow too.\n'
                ...
                self.lb50.setText('Error stop the experiment.')
